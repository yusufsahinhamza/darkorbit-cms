...
$clan = $mysqli->query('SELECT * FROM server_clan WHERE id = '.$clanId.'')->fetch_assoc();
...
<div class="card white-text grey darken-4  padding-15">
  ...
  <div class="row">
    ...
    <div class="col s6">
      ...
      <p>Company: <?php echo ($clan['factionID'] == 1 ? 'MMO' : ($clan['factionID'] == 2 ? 'EIC' : 'VRU')); ?></p>
      <p>Clan Rank: <?php echo ($clan['recruiting'] ? 'Recruiting' : 'Closed'); ?></p>
    </div>
      ...
      <div class="row">
        <form id="send_clan_application">
          ...
          <div class="input-field col s12">
            <textarea name="text" class="materialize-textarea white-text" placeholder="<?php echo ($pendingAlready ? 'Your application to this Clan is pending.' : 'Enter your application text here.'); ?>" <?php echo ($pendingAlready ? 'disabled' : ''); ?>></textarea>
          </div>
          <div class="input-field center col s12">
            <button class="btn grey darken-3 waves-effect waves-light col s12 <?php echo ($pendingAlready ? 'disabled' : ''); ?>">SEND</button>
          </div>
        </form>
      </div>
    </div>
